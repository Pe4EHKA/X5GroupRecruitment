...
3. Для ARM64 (linux-aarch64) теперь упакованы нативные бинари Vosk прямо в JAR, дополнительных действий не требуется.
4. Убедитесь, что на хосте достаточно дискового пространства (модель занимает сотни мегабайт).
5. Для корректного извлечения аудио внутри контейнера установлен `ffmpeg` (добавлено в `apps/backend/Dockerfile`). Никакие дополнительные CLI ставить не нужно.

Без каталога с моделью транскрибация не запустится и сервис вернёт ошибку конфигурации.
