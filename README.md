...
### 🎙️ Транскрибация видео (офлайн Vosk)
...
1. Скачайте подходящую Vosk-модель (например, `vosk-model-ru-0.10`) и распакуйте её в каталог `./transcription-model` в корне репозитория. Внутри каталога должны быть файлы `am`, `graph`, `conf` и т.п.
2. При запуске `docker compose up --build` каталог автоматически монтируется в контейнер по пути `/app/transcription-model`, а backend получает путь через переменные `APP_TRANSCRIPTION_LOCAL_MODEL_PATH` и `VOSK_MODEL_PATH`.
...
Без каталога `./transcription-model` транскрибация не запустится и сервис вернёт ошибку конфигурации.
